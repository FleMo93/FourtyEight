...
public class scr_Level_proc : MonoBehaviour
{
    ...
    scr_LevelManager_proc lvlManager;
    GameObject baseTile;

    private float groundUndergroundOffset = -0.25f;

    internal void InitLevel(int withd, int height, GameObject pBaseTile, GameObject parentElement, scr_LevelManager_proc lMng, Texture2D inputLayout = null)
    {
        ...
        for (int i = 0; i < TerrainHeight; i++)
        {
            for (int j = 0; j < TerrainWidth; j++)
            {
                ...
                if (ground_trs[j, i] == null)
                {
                    ...
                    ground_trs[j, i].parent = transform;
                }
                if (above_trs[j, i] == null)
                {
                    ...
                    above_trs[j, i].parent = transform;
                }
            }
        }
        ...
        for (int i = 0; i < TerrainHeight; i++)
        {
            for (int j = 0; j < TerrainWidth; j++)
            {
                ...
                else if (imgCol[j + i * TerrainWidth].isSameAs(Color_Ore_Stone))
                {
                    ...
                    ground_trs[j, i].transform.position = new Vector3((-TerrainWidth / 2) + j, groundUndergroundOffset, (-TerrainHeight / 2) + i);
                    ground_trs[j, i].parent = transform;
...
targetPlattfomr[x, y] == 18)
        {
            ...
            above_trs[x, y].parent = transform;
    ...
    public bool isSpaceFree(int x, int y, int width, int height)
    {
        x = x + TerrainWidth / 2;
        y = y + TerrainHeight / 2;
        ...
        {
            if (y + height <= TerrainHeight && x + width <= TerrainWidth)
            {
                for (int i_y = y; i_y < y+height; i_y++)
                {
                    for (int j_x = x; j_x < x+width; j_x++)
                    {
                        if (above[j_x, i_y] != 0)
                        {
                            return false;
